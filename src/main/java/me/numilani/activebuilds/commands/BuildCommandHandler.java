...
public class BuildCommandHandler {
  ...
  public void createBuilding(CommandSender sender, @Argument("name") String name, @Argument("type") String type)
      throws SQLException {
    // var x = plugin.cfg.getBuildings().stream().filter(bldg ->
    // bldg.Name.equals(type)).findFirst();
    ...
    sender.sendMessage(
        String.format("Building %s created! Designate input/output chests with /ab building update %s (input|output)",
            name, name, name));
  ...
  public void listBuildings(CommandSender sender) throws SQLException {
    ...
    for (var bldg : buildings) {
      ...
      if (bldg.getInputLocation() != null) {
        sender.sendMessage(String.format("  InLoc: (%s,%s,%s)", bldg.getInputLocation().getBlockX(),
            bldg.getInputLocation().getBlockY(), bldg.getInputLocation().getBlockZ()));
      }
      if (bldg.getOutputLocation() != null) {
        sender.sendMessage(String.format("  OutLoc: (%s,%s,%s)", bldg.getOutputLocation().getBlockX(),
            bldg.getOutputLocation().getBlockY(), bldg.getOutputLocation().getBlockZ()));
      }
    }
  }
  ...
  public void setBuildingInput(CommandSender sender, @Argument("name") String name) throws SQLException {
    ...
    plugin.dataSource.updateBuildingInputLocation(name,
        BlockLocationHelper.getSerializedLocation(target.getLocation()));
    sender.sendMessage(String.format("Input chest for building %s set to chest at %s, %s, %s", name,
  ...
  public void setBuildingOutput(CommandSender sender, @Argument("name") String name) throws SQLException {
    ...
    plugin.dataSource.updateBuildingOutputLocation(name,
        BlockLocationHelper.getSerializedLocation(target.getLocation()));
    sender.sendMessage(String.format("Output chest for building %s set to chest at %s, %s, %s", name,
        target.getLocation().getBlockX(), target.getLocation().getBlockY(), target.getLocation().getBlockZ()));
  }
  ...
  public void PrintDebugInfo(CommandSender sender) throws SQLException {
    ...
    sender.sendMessage(
        String.format("Last building update: %s", plugin.dataSource.getLastBuildingUpdateTime().toString()));
  }
