...
public class SqliteDataSourceConnector implements IDataSourceConnector {
  ...
  public void initDatabase() throws SQLException {
    ...
    statement.execute(
        "CREATE TABLE BuildingTypes(id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, inputs TEXT, outputs TEXT)");

    statement.execute(
        "CREATE TABLE Buildings(id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, type INTEGER, inputLocation TEXT, outputLocation TEXT)");
  }
  ...
  public Building getBuilding(String name) throws SQLException {
    var statement = conn
        .prepareStatement("SELECT id, name, type, inputLocation, outputLocation from Buildings WHERE name = ?");
    statement.setString(1, name);
    ...
    // This TYPE_UNAVAILABLE trick is a quick and dirty hack, replace this with
    // something more robust.
    // var type = plugin.cfg.getBuildings().stream().filter(a ->
    // a.Name.equals(storedType)).findFirst();
